...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    ghostDistances = util.PriorityQueue()
    for livingGhost in livingGhostPositionDistributions:
    	mostLikelyPos = livingGhost.argMax()
    	ghostDistances.push(mostLikelyPos, self.distancer.getDistance(pacmanPosition, mostLikelyPos))
    
    closestGhostPos = ghostDistances.pop()
    currentDistance = self.distancer.getDistance(pacmanPosition, closestGhostPos)
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
    	try:
    		newDistance = self.distancer.getDistance(closestGhostPos, Actions.getSuccessor(pacmanPosition, action))
